...
(function (NK) {
  ...
  var PROTECTED_SUPERADMIN_USERNAME = '9371519999';
  ...
  NK.MODULES['user-management'] = {
    ...
    _createUser: async function () {
      ...
      var username = String((c.querySelector('#usmNewUsername') || {}).value || '').trim();
      var displayName = String((c.querySelector('#usmNewDisplayName') || {}).value || '').trim();
      var role = (c.querySelector('#usmNewUserRole') || {}).value || 'admin';
      var password = String((c.querySelector('#usmNewPassword') || {}).value || '');
    ...
    _renderUsers: function (users) {
      ...
      tbody.innerHTML = users.map(function (user) {
        ...
        var isProtectedSuperadmin = String(user.username || '') === PROTECTED_SUPERADMIN_USERNAME;
        var statusPill = '<span class="usm-pill ' + (isActive ? 'usm-active' : 'usm-inactive') + '">' + esc(user.status || 'active') + '</span>';
        var enabledPermissions = String(user.role || '') === 'superadmin'
          ? PERMISSION_OPTIONS.map(function (opt) { return opt.key; })
          : (Array.isArray(user.permissions)
          ? user.permissions
          : Object.keys(user.permissions || {}).filter(function (key) { return !!user.permissions[key]; }));
        return '<tr>'
          ...
          + '<td>' + esc(user.role || '') + '</td>'
          + '<td>' + statusPill + '</td>'
          + '<td>' + esc(enabledPermissions.join(', ') || 'None') + '</td>'
          + '<td>'
          ...
          + (isProtectedSuperadmin
            ? '<span class="usm-muted">Protected</span>'

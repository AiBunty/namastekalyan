...
const nkResolvePublicUrl = (value) => {
...
if (heroMainVideo) {
    const heroPlaylist = [
        nkResolvePublicUrl('assets/01 (1).mp4'),
        nkResolvePublicUrl('assets/45.mp4'),
    ];
...
const nkEventsApiBase = (window.NK_DATA_API && typeof window.NK_DATA_API.resolveApiBaseForAction === 'function')
    ? String(window.NK_DATA_API.resolveApiBaseForAction('events_list') || '').split('?')[0]
    : ((window.NK_DATA_API && window.NK_DATA_API.phpApiUrl) ? String(window.NK_DATA_API.phpApiUrl || '').split('?')[0] : '');
...
const NK_EVENT_FALLBACK_IMAGE = nkResolvePublicUrl('assets/Hotel Pics/WhatsApp Image 2026-03-20 at 5.27.42 PM.jpeg?v=20260418-assetfix-3');
...
const syncHotelGalleryAssets = () => {
  const fallbackHotelAsset = nkResolvePublicUrl('assets/Hotel Pics/WhatsApp Image 2026-03-20 at 5.27.42 PM.jpeg?v=20260418-assetfix-3');
  const hotelAssetSelector = '[src^="assets/Hotel Pics/"], [poster^="assets/Hotel Pics/"], [data-full^="assets/Hotel Pics/"]';

  document.querySelectorAll(hotelAssetSelector).forEach((element) => {
    if (element.hasAttribute('src')) {
      const rawSrc = String(element.getAttribute('src') || '').trim();
      if (rawSrc) {
        element.setAttribute('src', nkResolvePublicUrl(rawSrc));
        if (element.tagName === 'IMG') {
          element.onerror = function onHotelImageError() {
            if (this.dataset.fallbackApplied === '1') {
              this.onerror = null;
              return;
            }
            this.dataset.fallbackApplied = '1';
            this.src = fallbackHotelAsset;
          };
        }
      }
    }
    ...
    if (element.hasAttribute('poster')) {
      const rawPoster = String(element.getAttribute('poster') || '').trim();
      if (rawPoster) {
        element.setAttribute('poster', nkResolvePublicUrl(rawPoster));
      }
    }

    if (element.hasAttribute('data-full')) {
      const rawFull = String(element.getAttribute('data-full') || '').trim();
      if (rawFull) {
        element.setAttribute('data-full', nkResolvePublicUrl(rawFull));
      }
    }
  });
};

syncHotelGalleryAssets();
        ...
        return null;
            ...
            if (parsed && typeof parsed === 'object') {
                ...
                const items = Array.isArray(parsed.items)
                    ? parsed.items
                    : (parsed.payload && Array.isArray(parsed.payload.items) ? parsed.payload.items : []);

                return {
                    ...parsed,
                    items,
                    count: typeof parsed.count === 'number' ? parsed.count : items.length,
                    action: parsed.action || 'events_list',
                    ok: parsed.ok !== false,
                };
            }
    ...
    try {
      ...
      sessionStorage.setItem(NK_EVENTS_CACHE_KEY, JSON.stringify({
        timestamp: Date.now(),
                ...
                action: payload.action || 'events_list',
                ok: payload.ok !== false,
                data: payload
      }));
    } catch (err) {
...
const clearSharedEventsCache = () => {
    nkEventsCache.clear();
    nkSelectedEvent.clear();
};
...
const buildEventsListingUrl = (eventId) => {
    const id = String(eventId || '').trim();
    return id
        ? nkResolvePublicUrl(`events/?eventId=${encodeURIComponent(id)}`)
        : nkResolvePublicUrl('events/');
};
...
const fetchEventsApi = async (action, params = {}) => {
    const resolvedBase = (window.NK_DATA_API && typeof window.NK_DATA_API.resolveApiBaseForAction === 'function')
        ...
        : nkEventsApiBase;
    const fallbackBase = (window.NK_DATA_API && window.NK_DATA_API.phpApiUrl)
        ? String(window.NK_DATA_API.phpApiUrl).split('?')[0].trim()
        : nkEventsApiBase;
...
const ensureEventPopupShell = () => {
    ...
    const closeModal = () => {
        ...
        scrollToFoodMenuLandingTarget('smooth');
    };
...
const getHomeFoodMenuLandingTarget = () => {
    return document.getElementById('foodMenuCard') || document.getElementById('menu');
};

const shouldPreserveExplicitHomeHash = () => {
    const hash = String(window.location.hash || '').trim();
    return !!hash && hash !== '#foodMenuCard' && hash !== '#menu';
};

const scrollHomeToFoodMenuLandingTarget = (behavior = 'auto') => {
    if (shouldPreserveExplicitHomeHash()) return;

    const target = getHomeFoodMenuLandingTarget();
    if (!target) return;
...
const scheduleInitialHomeLandingAlignment = () => {
    if (shouldPreserveExplicitHomeHash()) return;
    ...
    [80, 260, 700, 1400].forEach((delayMs) => {
        window.setTimeout(() => {
            scrollHomeToFoodMenuLandingTarget('auto');
        }, delayMs);
    });
...
const initEventsExperience = () => {
    ...
    // On landing page load, keep the homepage aligned to the Food Menu card.
    scheduleInitialHomeLandingAlignment();

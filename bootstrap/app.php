...
define('NK_BASE_DIR', dirname(__DIR__));
define('NK_SRC_DIR',  NK_BASE_DIR . '/app');
...
$activeProfile = strtolower(trim((string) ($_ENV['NK_ENV_PROFILE'] ?? '')));
if (in_array($activeProfile, ['local', 'live'], true)) {
    $profileSuffix = strtoupper($activeProfile);
    $profileKeys = [
        'APP_ENV',
        'APP_URL',
        'APP_PUBLIC_SITE_URL',
    ...
    foreach ($profileKeys as $baseKey) {
        $profileKey = $baseKey . '_' . $profileSuffix;
        $profileValue = trim((string) ($_ENV[$profileKey] ?? ''));
        if ($profileValue === '') {
            continue;
        }

        $_ENV[$baseKey] = $profileValue;
        putenv($baseKey . '=' . $profileValue);
    }
}

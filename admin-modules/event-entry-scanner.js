  ...
  NK.MODULES['event-entry-scanner'] = {
    ...
    init: function (container, authClient, user) {
      ...
      styleEl.textContent = [
        ...
        '.scn-layout { display: grid; grid-template-columns: minmax(300px,380px) minmax(0,1fr); gap: 18px; align-items: start; }',
        ...
        '.scn-queue-wrap table { width:100%; border-collapse:collapse; min-width:880px; }',
        ...
        '@media (max-width:980px) { .scn-layout { grid-template-columns:1fr; } }',
      ...
      function renderQueue() {
        ...
        queueBody.innerHTML = queue.map(function (item) {
          ...
          return [
            ...
            '<td data-label="Event"><div class="scn-guest-meta"><strong>', esc(item.eventTitle || item.eventId || '-'), '</strong><small>', esc(eventMeta), '</small></div></td>',
            '<td data-label="Type">', esc(bookingType(item)), '</td>',
            '<td data-label="Source">', esc(item.source || '-'), '</td>',
            '<td data-label="Arrived Now"><input type="number" min="1" step="1" style="max-width:95px;" data-admit-count="', esc(item.id), '" value="', esc(admitVal), '"', admitDisabled ? ' disabled' : '', '></td>',
        ...
        try {
          var response = await authClient.apiPost({
            ...
            scans: valid.map(function (i) {
              return { scanText: i.rawScan, admittedCount: Math.max(1, Math.floor(Number(i.admittedCount || 1) || 1)) };
            })
          });

...
**Live DB host rule**:
- The production PHP runtime must use the hosting provider's internal MySQL host `sdb-53.hosting.stackcp.net` on port `3306`.
- Do not assume the external workstation-facing DB host will work from the server itself.
- Keep the server's `.env` authoritative for live DB settings; deploy artifacts should not overwrite it.

...
class EventTransactionRepository
{
    ...
    public function findLatestForEventAndCustomer(string $eventId, string $email, string $phone): ?array
    {
        ...
        $stmt->execute([
            ':event_id' => $eventId,
                        ':email_check' => trim($email),
                        ':email_match' => trim($email),
                        ':phone_check' => trim($phone),
                        ':phone_match' => trim($phone),
        ]);
        $row = $stmt->fetch();
        return $row ?: null;
    }
    ...
    public function listReportRows(?string $eventId = null): array
    {
        $sql = 'SELECT * FROM event_transactions';
        $params = [];

        if ($eventId !== null && trim($eventId) !== '') {
            $sql .= ' WHERE event_id = :event_id';
            $params[':event_id'] = trim($eventId);
        }
    ...
    public function updateCheckinState(
        ...
        $stmt->execute([
            ...
            ':checked_in_count' => max(0, $checkedInCount),

...
class EventRepository
{
    ...
    public function deleteByEventId(string $eventId): void
    {
        ...
        try {
            $deleteLedger = $this->db->prepare('DELETE FROM admin_cash_ledger WHERE event_id = :event_id');
            $deleteLedger->execute([':event_id' => $eventId]);

            $deleteTransactions = $this->db->prepare('DELETE FROM event_transactions WHERE event_id = :event_id');
            $deleteTransactions->execute([':event_id' => $eventId]);

            $deleteEvent = $this->db->prepare('DELETE FROM events WHERE event_id = :event_id');

...
class AuthService
{
    ...
    public function listUsers(array $data): array
    {
        ...
        return [
            ...
            // Keep both keys for backward compatibility with existing frontend code.
            'users'  => $users,
            'items'  => $users,
        ];
    }
    ...
    public static function toPublicUser(array $row): array
    {
        ...
        return [
            ...
            'permissions'         => self::publicPermissions(is_array($permissions) ? $permissions : [], $role),
    ...
    {
        $permissions = self::normalizePermissionMap($permissions);
        $clean = [];
        foreach (Constants::ADMIN_PERMISSION_KEYS as $key) {
    ...
    private static function normalizePermissionMap(array $permissions): array
    {
    ...
    private static function publicPermissions(array $permissions, string $role): array
    {
        if ($role === 'superadmin') {
            return Constants::ADMIN_PERMISSION_KEYS;
        }

        $clean = self::sanitizePermissions($permissions);
        $granted = [];
        foreach ($clean as $key => $allowed) {
            if ($allowed) {
                $granted[] = $key;
            }
        }

...
class EventTransactionRepository
{
    ...
    /**
     * Persist gate-entry/check-in state for a transaction.
     */
    public function updateCheckinState(
        ...
        $sql = 'UPDATE event_transactions
                SET status = :status,
                    ...
                    guest_passes_json = :guest_passes_json
                WHERE transaction_id = :transaction_id';
        ...
        $stmt->execute([
            ...
            ':guest_passes_json'=> $guestPasses === null
                ? null
                : json_encode($guestPasses, JSON_UNESCAPED_UNICODE),
            ':transaction_id'   => $transactionId,
        ]);
    }
}
